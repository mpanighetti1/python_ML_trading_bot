...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_1_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_1_{testing_stage}"  # Use the testing_stage in study name
...
ma_long = ma_slow <= ma_mid <= ma_fast
ma_short = ma_slow >= ma_mid >= ma_fast
