...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_2_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_2_{testing_stage}"  # Use the testing_stage in study name
...
def save_selected_variables(selected_params):
    # Load existing data
    try:
        with open('saved_variables_2.json', 'r') as f:
    ...
    with open('saved_variables_2.json', 'w') as f:
