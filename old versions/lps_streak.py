...
HTF_index = 0 #trial.suggest_int('HTF_index', 0, 6)  
...
LiquidityPurgeStrategy = vbt.IF.from_expr(
    ...
    # Williams Fractals
    first_target_length_input=200,
    willfrac_period=4,
    short_target_multiple=0,
    long_target_multiple=0,
    short_entry_multiple=0,
    long_entry_multiple=0,
...
def objective(trial, LTF_high, 
    ...
    global sl_multiplier, sl_period, rsi_lower_threshold, rsi_period, rsi_upper_threshold, \
       first_target_length_input, short_target_multiple, long_target_multiple, \
    ...
    if (testing_stage <= 5):
        name_1, variable_1 = chosen_functions[0](trial)   
        name_2, variable_2 = chosen_functions[1](trial)   
        name_3, variable_3 = chosen_functions[2](trial)   
        name_4, variable_4 = chosen_functions[3](trial)   
        name_5, variable_5 = chosen_functions[4](trial)   
        globals()[name_1] = variable_1
        globals()[name_2] = variable_2 
        globals()[name_3] = variable_3 
        globals()[name_4] = variable_4
        globals()[name_5] = variable_5
    elif (testing_stage > 5 and testing_stage <= 15):
        name_1, variable_1 = chosen_functions[0](trial)   
        name_2, variable_2 = chosen_functions[1](trial) 
        globals()[name_1] = variable_1   
        globals()[name_2] = variable_2   
    ...
    lps = LiquidityPurgeStrategy.run(LTF_high, LTF_low, LTF_close, # calls LTF_high, LTF_low, LTF_close
                                    sl_period=sl_period,
                                    sl_multiplier=sl_multiplier,
                                    first_target_length_input=first_target_length_input,
                                    willfrac_period=will_frac_period,
    ...
    lps_short_entry_min_value = np.around(lps.short_entry_min_value.shift(1).fillna(placeholder), 7)
    lps_short_entry_mid_value = np.around(lps.short_entry_mid_value.shift(1).fillna(placeholder), 7)
    lps_short_entry_max_value = np.around(lps.short_entry_max_value.shift(1).fillna(placeholder), 7)
    
    lps_long_entry_min_value = np.around(lps.long_entry_min_value.shift(1).fillna(placeholder), 7)
    lps_long_entry_mid_value = np.around(lps.long_entry_mid_value.shift(1).fillna(placeholder), 7)
    lps_long_entry_max_value = np.around(lps.long_entry_max_value.shift(1).fillna(placeholder), 7)
    
    lps_short_take_profit_value = np.around(lps.short_take_profit_value.shift(1).fillna(placeholder), 7)
    lps_long_take_profit_value = np.around(lps.long_take_profit_value.shift(1).fillna(placeholder), 7)
    
    lps_short_sl_atr_value = np.around(lps.short_sl_atr_value.shift(1).fillna(placeholder), 7)
    lps_long_sl_atr_value = np.around(lps.long_sl_atr_value.shift(1).fillna(placeholder), 7)
    ...
    for col in tqdm(range(LTF_open_np.shape[1]), desc='Processing columns'): 
        ...
        short_entries_1, short_entries_2, short_entries_3, \
        short_stop_loss_hit_np, long_stop_loss_hit_np, \
        short_take_profit_hit_np, long_take_profit_hit_np = clean_signals_nb(short_entry_1_default_cond_np,
                                                                            short_entry_2_default_cond_np,
                                                                            short_entry_3_default_cond_np,
                                                                            short_take_profit_value_raw,
                                                                            long_entry_1_default_cond_np,
    ...
    for col in tqdm(range(LTF_close_np.shape[1]), desc='Processing columns'):
    ...
    if pf.trades.count() > 0:        
        ...
        return round(pf.total_return, 7), pf.trades.count()
...
def suggest_sl_multiplier_precise(trial):
    return ("sl_multiplier", round(trial.suggest_float("sl_multiplier", 1.0, 21.0, step=0.1), 2))

def suggest_will_frac_period_precise(trial):
    return ("will_frac_period", trial.suggest_int("will_frac_period", 2, 30))

def suggest_first_target_length_input_precise(trial):
    return ("first_target_length_input", trial.suggest_int("first_target_length_input", 10, 20010, step=1))

def suggest_rsi_lower_threshold_precise(trial):
    return ("rsi_lower_threshold", trial.suggest_int("rsi_lower_threshold", 1, 99, step=1))

def suggest_rsi_upper_threshold_precise(trial):
    return ("rsi_upper_threshold", trial.suggest_int("rsi_upper_threshold", 1, 99, step=1))

def suggest_short_target_multiple_precise(trial):
    return ("short_target_multiple", round(trial.suggest_float("short_target_multiple", 0.0, 0.01, step=0.00001), 6))

def suggest_long_target_multiple_precise(trial):
    return ("long_target_multiple", round(trial.suggest_float("long_target_multiple", 0.0, 0.01, step=0.00001), 6))

def suggest_short_entry_multiple_precise(trial):
    return ("short_entry_multiple", round(trial.suggest_float("short_entry_multiple", 0.0, 0.01, step=0.00001), 6))

def suggest_long_entry_multiple_precise(trial):
    return ("long_entry_multiple", round(trial.suggest_float("long_entry_multiple", 0.0, 0.01, step=0.00001), 6))

def suggest_sl_period_precise(trial):
    return ("sl_period", trial.suggest_int("sl_period", 10, 1000, step=1))

def suggest_rsi_period_precise(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 10, 1000, step=1))

################################# Mid ######################################

def suggest_sl_multiplier_mid(trial):
    return ("sl_multiplier", round(trial.suggest_float("sl_multiplier", 1.0, 21.0, step=0.5), 2))

def suggest_will_frac_period_mid(trial):
    return ("will_frac_period", trial.suggest_int("will_frac_period", 2, 30))

def suggest_first_target_length_input_mid(trial):
    return ("first_target_length_input", trial.suggest_int("first_target_length_input", 10, 20010, step=100))

def suggest_rsi_lower_threshold_mid(trial):
    return ("rsi_lower_threshold", trial.suggest_int("rsi_lower_threshold", 1, 99, step=2))

def suggest_rsi_upper_threshold_mid(trial):
    return ("rsi_upper_threshold", trial.suggest_int("rsi_upper_threshold", 1, 99, step=2))

def suggest_short_target_multiple_mid(trial):
    return ("short_target_multiple", round(trial.suggest_float("short_target_multiple", 0.0, 0.01, step=0.00005), 6))

def suggest_long_target_multiple_mid(trial):
    return ("long_target_multiple", round(trial.suggest_float("long_target_multiple", 0.0, 0.01, step=0.00005), 6))

def suggest_short_entry_multiple_mid(trial):
    return ("short_entry_multiple", round(trial.suggest_float("short_entry_multiple", 0.0, 0.01, step=0.00005), 6))

def suggest_long_entry_multiple_mid(trial):
    return ("long_entry_multiple", round(trial.suggest_float("long_entry_multiple", 0.0, 0.01, step=0.00005), 6))

def suggest_sl_period_mid(trial):
    return ("sl_period", trial.suggest_int("sl_period", 25, 1000, step=25))

def suggest_rsi_period_mid(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 25, 1000, step=25))

################################# Wide ######################################

def suggest_sl_multiplier_wide(trial):
    return ("sl_multiplier", round(trial.suggest_float("sl_multiplier", 1.0, 21.0, step=2.0), 2))

def suggest_will_frac_period_wide(trial):
    return ("will_frac_period", trial.suggest_int("will_frac_period", 2, 30))

def suggest_first_target_length_input_wide(trial):
    return ("first_target_length_input", trial.suggest_int("first_target_length_input", 10, 20010, step=500))

def suggest_rsi_lower_threshold_wide(trial):
    return ("rsi_lower_threshold", trial.suggest_int("rsi_lower_threshold", 1, 99, step=2))

def suggest_rsi_upper_threshold_wide(trial):
    return ("rsi_upper_threshold", trial.suggest_int("rsi_upper_threshold", 1, 99, step=2))

def suggest_short_target_multiple_wide(trial):
    return ("short_target_multiple", round(trial.suggest_float("short_target_multiple", 0.0, 0.01, step=0.0001), 6))

def suggest_long_target_multiple_wide(trial):
    return ("long_target_multiple", round(trial.suggest_float("long_target_multiple", 0.0, 0.01, step=0.0001), 6))

def suggest_short_entry_multiple_wide(trial):
    return ("short_entry_multiple", round(trial.suggest_float("short_entry_multiple", 0.0, 0.01, step=0.0001), 6))

def suggest_long_entry_multiple_wide(trial):
    return ("long_entry_multiple", round(trial.suggest_float("long_entry_multiple", 0.0, 0.01, step=0.0001), 6))

def suggest_sl_period_wide(trial):
    return ("sl_period", trial.suggest_int("sl_period", 50, 1000, step=50))

def suggest_rsi_period_wide(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 50, 1000, step=50))
...
def run_optimization(testing_stage, LTF_high, 
    ...
    file_path = f"./op_9_16_{testing_stage}.log"
    ...
    study_name = f"op_9_16_{testing_stage}"  # Use the testing_stage in study name
...
def save_selected_variables(selected_params):
    # Load existing data
    try:
        with open('saved_variables.json', 'r') as f:
...
saved_objective_value: 0.0194693

Top trial information from the last study:

Objective values:
Objective 1: 0.0194693
Objective 2: 209.0

Constraint values:
Constraint 1: -0.013778515681692954
Constraint 2: -321.2511789525937
Constraint 3: -9.0
Constraint 4: -0.01946927521601459
'''

# Starting Testing Parameters:
testing_stage = 1
saved_first_target_length_input = 500
saved_rsi_lower_threshold = 100
saved_rsi_period = 200
saved_rsi_upper_threshold = 0
saved_sl_multiplier = 4.0
saved_sl_period = 200
saved_short_target_multiple = 0.0
saved_long_target_multiple =  0.0
saved_short_entry_multiple =  0.0
saved_long_entry_multiple =   0.0
saved_will_frac_period = 4
...
#1 at time
suggestion_functions_precise = [
    suggest_sl_multiplier_precise, 
    suggest_will_frac_period_precise,
    suggest_first_target_length_input_precise,
    suggest_rsi_lower_threshold_precise,
    suggest_rsi_upper_threshold_precise,
    #suggest_short_target_multiple_precise,
    #suggest_long_target_multiple_precise,
    #suggest_short_entry_multiple_precise,
    #suggest_long_entry_multiple_precise,
    suggest_sl_period_precise,
    suggest_rsi_period_precise
]

# 2 at time.
suggestion_functions_mid = [
    suggest_sl_multiplier_mid, 
    suggest_will_frac_period_mid,
    suggest_first_target_length_input_mid,
    suggest_rsi_lower_threshold_mid,
    suggest_rsi_upper_threshold_mid,
    #suggest_short_target_multiple_mid,
    #suggest_long_target_multiple_mid,
    #suggest_short_entry_multiple_mid,
    #suggest_long_entry_multiple_mid,
    suggest_sl_period_mid,
    suggest_rsi_period_mid
]

# run all.
suggestion_functions_wide = [
    suggest_sl_multiplier_wide, 
    suggest_will_frac_period_wide,
    suggest_first_target_length_input_wide,
    suggest_rsi_lower_threshold_wide,
    suggest_rsi_upper_threshold_wide,
    #suggest_short_target_multiple_wide,
    #suggest_long_target_multiple_wide,
    #suggest_short_entry_multiple_wide,
    #suggest_long_entry_multiple_wide,
    suggest_sl_period_wide,
    suggest_rsi_period_wide
]

if (testing_stage <= 5):
    chosen_functions = random.sample(suggestion_functions_wide, 7)
elif (testing_stage > 5 and testing_stage <= 15):
    chosen_functions = random.sample(suggestion_functions_mid, 3)
else:
    chosen_functions = random.sample(suggestion_functions_precise, 1)
...
if __name__ == '__main__':       
    ...
    if best_params and top_trial_info:
        ...
        if saved_value is None or top_trial_info["objectives"][0] > saved_value:
            ...
            file_name = f"{study.study_name}.txt"
