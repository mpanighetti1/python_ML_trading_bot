...
def clean_signals_nb(   long_combined_exit_cond_np,
                        short_combined_exit_cond_np,
                        short_entry_1_no_ma_cond_np,
    ...
    long_stop_loss_atr_value_np = np.full_like(m1_open_np, np.nan)
    short_stop_loss_atr_value_np = np.full_like(m1_open_np, np.nan)    
    long_stop_loss_dir_value_np = np.full_like(m1_open_np, np.nan)
    short_stop_loss_dir_value_np = np.full_like(m1_open_np, np.nan)
    ...
    long_stop_loss_atr_hit_np = np.full_like(m1_open_np, False, dtype=np.bool_)
    short_stop_loss_atr_hit_np = np.full_like(m1_open_np, False, dtype=np.bool_)
    long_stop_loss_dir_hit_np = np.full_like(m1_open_np, False, dtype=np.bool_)
    short_stop_loss_dir_hit_np = np.full_like(m1_open_np, False, dtype=np.bool_)
    ...
    for col in prange(m1_open_np.shape[1]):
        ...
        for i in range(m1_open_np.shape[0]):
            ...
            if np.isnan(long_stop_loss_atr_value_np[i, col]): # If the value is NaN
                # Set the value to the previous value (to be overwritten if entry occurs)
                long_stop_loss_atr_value_np[i, col] = long_stop_loss_atr_value_np[i - 1, col] 
            if np.isnan(short_stop_loss_atr_value_np[i, col]):
                short_stop_loss_atr_value_np[i, col] = short_stop_loss_atr_value_np[i - 1, col]
            ...
            (not np.isnan(long_sl_atr_value_np[i, col])):
                ...
                long_stop_loss_atr_value_np[i, col] = min(long_stop_1, min(long_stop_2, long_stop_3))
            ...
            if long_any and (m1_high_np[i, col] >= long_take_profit_value_np[i, col]):
                long_take_profit_hit_np[i, col] = True
                l1 = False
                l2 = False
                l3 = False
            '''
            # Long Stop Loss Exit (ATR Stop)       
            if long_any and (m1_low_np[i, col] <= long_stop_loss_atr_value_np[i, col]):
                long_stop_loss_atr_hit_np[i, col] = True
                l1 = False
                l2 = False
                l3 = False
            '''
            # Long Stop Loss Exit (Direction Change)        
            if long_any and (short_combined_exit_cond_np[i, col]):
                long_stop_loss_dir_value_np[i, col] = m1_close_np[i, col]
                long_stop_loss_dir_hit_np[i, col] = True
                l1 = False
                l2 = False
                l3 = False
            ...
            (not np.isnan(short_sl_atr_value_np[i, col])):
                ...
                short_stop_loss_atr_value_np[i, col] = max(short_stop_1, max(short_stop_2, short_stop_3))
            ...
            if short_any and (m1_low_np[i, col] <= short_take_profit_value_np[i, col]) :
                short_take_profit_hit_np[i, col] = True
                s1 = False
                s2 = False
                s3 = False
            '''
            # Short Stop Loss Exit (ATR Stop)
            if short_any and (m1_high_np[i, col] >= short_stop_loss_atr_value_np[i, col]):
                short_stop_loss_atr_hit_np[i, col] = True
                s1 = False
                s2 = False
                s3 = False
            '''
            # Short Stop Loss Exit (Direction Change)
            if short_any and (long_combined_exit_cond_np[i, col]):
                short_stop_loss_dir_value_np[i, col] = m1_close_np[i, col]
                short_stop_loss_dir_hit_np[i, col] = True
                s1 = False
                s2 = False
                s3 = False
            
    # Returns
    return long_stop_loss_atr_value_np, short_stop_loss_atr_value_np, \
           long_stop_loss_dir_value_np, short_stop_loss_dir_value_np, \
           ...
           short_stop_loss_atr_hit_np, long_stop_loss_atr_hit_np, \
           short_stop_loss_dir_hit_np, long_stop_loss_dir_hit_np, \
...
def pipeline_6_nb(m1_open_np,
    ...
    long_stop_loss_atr_hit_np,
    long_stop_loss_atr_value_np,
    long_stop_loss_dir_hit_np,
    long_stop_loss_dir_value_np,
    ...
    short_stop_loss_atr_hit_np,
    short_stop_loss_atr_value_np,
    short_stop_loss_dir_hit_np,
    short_stop_loss_dir_value_np,
    ...
    for col in prange(m1_open_np.shape[1]):                
        ...
        for i in range(m1_open_np.shape[0]):  
            ...
            order_triggers = [  # Close Opposing Position
                                ...
                                ((long_stop_loss_atr_hit_np[i, col] and exec_state.position > 0), 
                                long_stop_loss_atr_value_np[i, col], 0, 1, 8),
                                
                                ((long_stop_loss_dir_hit_np[i, col] and exec_state.position > 0), 
                                long_stop_loss_dir_value_np[i, col], 0, 1, 8),
                                
                                ((short_take_profit_hit_np[i, col] and exec_state.debt > 0), 
                                short_take_profit_value_np[i, col], 0, 0, 8),
                                
                                ((short_stop_loss_atr_hit_np[i, col] and exec_state.debt > 0), 
                                short_stop_loss_atr_value_np[i, col], 0, 0, 8),
                                
                                ((short_stop_loss_dir_hit_np[i, col] and exec_state.debt > 0), 
                                short_stop_loss_dir_value_np[i, col], 0, 0, 8)
                            ] 
...
def objective(trial, LTF_high, 
    ...
    global sl_multiplier, sl_period, \
        ...
        rsi_exit_lower_threshold, rsi_exit_upper_threshold, \
    ...
    rsi_exit_lower_threshold = saved_rsi_exit_lower_threshold
    ...
    rsi_exit_upper_threshold = saved_rsi_exit_upper_threshold
    ...
    if str(testing_stage).endswith(('1','3','5','7','9')):    
        # Call the functions and assign their results dynamically
        for i in range(2):
            name, variable = chosen_functions_core[i](trial)
            globals()[name] = variable

        # Same for Features MA
        for i in range(2):
            name, variable = chosen_functions_ma[i](trial)
            globals()[name] = variable

    elif str(testing_stage).endswith(('0','2','4','6','8')):
        # Same for Features Entry RSI
        for i in range(3):
            name, variable = chosen_functions_entry_rsi[i](trial)
            globals()[name] = variable

        # Same for Features Exit RSI
        for i in range(3):
            name, variable = chosen_functions_exit_rsi[i](trial)
            globals()[name] = variable
    ...
    ma = MultiMA.run(HTF_close, # calls LTF_close
    ...
    rsi_exit = RelativeStrengthIndex_exit.run(LTF_close, # calls LTF_close
                        rsi_period=rsi_period,
                        rsi_exit_upper_threshold=rsi_exit_upper_threshold,
                        rsi_exit_lower_threshold=rsi_exit_lower_threshold,                   
                        execute_kwargs=dict(                                        
                                        engine='dask',
                                        chunk_len='auto',
                                        show_progress=False
    ...
    rsi_exit_above_upper_threshold = rsi_exit.rsi_exit_above_upper_threshold.shift(1).fillna(placeholder)
    rsi_exit_below_lower_threshold = rsi_exit.rsi_exit_below_lower_threshold.shift(1).fillna(placeholder)
    
    ma_long = ma.ma_long.shift(1).fillna(placeholder)
    ma_short = ma.ma_short.shift(1).fillna(placeholder)
    ma_green_over_red = ma.ma_green_over_red.shift(1).fillna(placeholder)
    ma_green_under_red = ma.ma_green_under_red.shift(1).fillna(placeholder)
    ...
    lps_short_sl_atr_value = lps_LTF.short_sl_atr_value.shift(1).fillna(placeholder)
    lps_long_sl_atr_value = lps_LTF.long_sl_atr_value.shift(1).fillna(placeholder)
    ...
    ma_green_over_red_m1 = ma_green_over_red.reindex(m1_open.index).ffill()
    ma_green_over_red_m1 = ma_green_over_red_m1.bfill()
    ma_green_under_red_m1 = ma_green_under_red.reindex(m1_open.index).ffill()
    ma_green_under_red_m1 = ma_green_under_red_m1.bfill()
    ...
    rsi_exit_above_upper_threshold_m1 = rsi_exit_above_upper_threshold.reindex(LTF_close.index).ffill()
    rsi_exit_above_upper_threshold_m1 = rsi_exit_above_upper_threshold_m1.bfill()
    rsi_exit_below_lower_threshold_m1 = rsi_exit_below_lower_threshold.reindex(LTF_close.index).ffill()
    rsi_exit_below_lower_threshold_m1 = rsi_exit_below_lower_threshold_m1.bfill()
    ...
    lps_short_sl_atr_value_m1 = lps_short_sl_atr_value.reindex(m1_open.index).ffill()
    lps_long_sl_atr_value_m1 = lps_long_sl_atr_value.reindex(m1_open.index).ffill()
    ...
    ma_green_over_red_m1 = ma_green_over_red_m1.replace(placeholder, np.nan)
    ma_green_under_red_m1 = ma_green_under_red_m1.replace(placeholder, np.nan)
    ...
    rsi_exit_above_upper_threshold_m1 = rsi_exit_above_upper_threshold_m1.replace(placeholder, np.nan)
    rsi_exit_below_lower_threshold_m1 = rsi_exit_below_lower_threshold_m1.replace(placeholder, np.nan)
    ...
    short_rsi_exit_cond = rsi_exit_below_lower_threshold_m1.astype(bool) 
    short_combined_entry_cond = short_ma_cond & short_rsi_entry_cond
    short_combined_exit_cond = ma_green_over_red_m1 & short_rsi_exit_cond
    ...
    long_rsi_exit_cond = rsi_exit_above_upper_threshold_m1.astype(bool)
    long_combined_entry_cond = long_ma_cond & long_rsi_entry_cond
    long_combined_exit_cond = ma_green_under_red_m1 & long_rsi_exit_cond
    ...
    short_combined_exit_cond_np = short_combined_exit_cond.to_numpy()
    short_ma_cond_np = short_ma_cond.to_numpy()
    ...
    long_combined_exit_cond_np = long_combined_exit_cond.to_numpy()
    long_ma_cond_np = long_ma_cond.to_numpy()
    ...
    short_combined_exit_cond_np = ensure_2d(short_combined_exit_cond_np)
    short_ma_cond_np = ensure_2d(short_ma_cond_np)
    ...
    long_combined_exit_cond_np = ensure_2d(long_combined_exit_cond_np)
    long_ma_cond_np = ensure_2d(long_ma_cond_np)
    ...
    long_stop_loss_atr_value_np, short_stop_loss_atr_value_np, \
    long_stop_loss_dir_value_np, short_stop_loss_dir_value_np, \
    long_take_profit_value_np, short_take_profit_value_np, \
    long_entries_1, long_entries_2, long_entries_3, \
    short_entries_1, short_entries_2, short_entries_3, \
    short_stop_loss_atr_hit_np, long_stop_loss_atr_hit_np, \
    short_stop_loss_dir_hit_np, long_stop_loss_dir_hit_np, \
    short_take_profit_hit_np, long_take_profit_hit_np = clean_signals_nb(   long_combined_exit_cond_np,
                                                                            short_combined_exit_cond_np,
                                                                            short_entry_1_no_ma_cond_np,
                                                                            short_entry_2_no_ma_cond_np,
                                                                            short_entry_3_no_ma_cond_np,
    ...
    long_stop_loss_atr_hit_np = ensure_2d(long_stop_loss_atr_hit_np) 
    long_stop_loss_atr_value_np = ensure_2d(long_stop_loss_atr_value_np)
    long_stop_loss_dir_hit_np = ensure_2d(long_stop_loss_dir_hit_np) 
    long_stop_loss_dir_value_np = ensure_2d(long_stop_loss_dir_value_np)
    ...
    short_stop_loss_atr_hit_np = ensure_2d(short_stop_loss_atr_hit_np)
    short_stop_loss_atr_value_np = ensure_2d(short_stop_loss_atr_value_np)
    short_stop_loss_dir_hit_np = ensure_2d(short_stop_loss_dir_hit_np)
    short_stop_loss_dir_value_np = ensure_2d(short_stop_loss_dir_value_np)
    ...
    if pf.trades.count() > 0:        
        ...
        max_drawdown = float(-0.005 + -(pf.max_drawdown))
        filtered_trade_duration = float(avg_trade_duration - 600) # 10 Hours
        trade_count = float(400 - pf.trades.count())
...
def suggest_sl_multiplier(trial): #Not Rounding
    return ("sl_multiplier", trial.suggest_int("sl_multiplier", 2, 202, step=5)) #Not Rounding
...
def suggest_ma_fast_period(trial):
    return ("ma_fast_period", trial.suggest_int("ma_fast_period", 5, ma_mid_period - 5, step=1))

def suggest_ma_mid_period(trial):
    return ("ma_mid_period", trial.suggest_int("ma_mid_period", ma_fast_period + 5, ma_slow_period - 5, step=1))

def suggest_ma_slow_period(trial):
    return ("ma_slow_period", trial.suggest_int("ma_slow_period", ma_mid_period + 5, 300, step=1))

def suggest_first_target_length_input(trial):
    return ("first_target_length_input", trial.suggest_int("first_target_length_input", 10, 1000, step=10))
...
def suggest_rsi_exit_lower_threshold(trial):
    return ("rsi_exit_lower_threshold", trial.suggest_int("rsi_exit_lower_threshold", 0, 100))

def suggest_rsi_exit_upper_threshold(trial):
    return ("rsi_exit_upper_threshold", trial.suggest_int("rsi_exit_upper_threshold", 0, 100))

def suggest_short_target_multiple(trial):
    return ("short_target_multiple", trial.suggest_float("short_target_multiple", 0.0, 0.01, step=0.00001))

def suggest_long_target_multiple(trial):
    return ("long_target_multiple", trial.suggest_float("long_target_multiple", 0.0, 0.01, step=0.00001))

def suggest_short_entry_multiple(trial):
    return ("short_entry_multiple", trial.suggest_float("short_entry_multiple", 0.0, 0.01, step=0.00001))

def suggest_long_entry_multiple(trial):
    return ("long_entry_multiple", trial.suggest_float("long_entry_multiple", 0.0, 0.01, step=0.00001))

def suggest_sl_period(trial):
    return ("sl_period", trial.suggest_int("sl_period", 10, 3000, step=10))

def suggest_rsi_period(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 7, 21, step=7))
...
def filter_and_sort_trials(study: opt.study.Study) -> list:
    ...
    filtered_trials = [
        ...
        if t.values[1] >= 400 and
        t.values[2] <= 600 # Narrow Filter
    ]
...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_testnew_1_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_testnew_1_{testing_stage}"  # Use the testing_stage in study name
...
def save_selected_variables(selected_params):
    # Load existing data
    try:
        with open('saved_variables_testnew_1.json', 'r') as f:
    ...
    with open('saved_variables_testnew_1.json', 'w') as f:
...
# Core
saved_first_target_length_input = 300
saved_rsi_entry_lower_threshold = 26
saved_rsi_exit_lower_threshold = 5
saved_rsi_period = 21
saved_rsi_entry_upper_threshold = 52
saved_rsi_exit_upper_threshold = 5
saved_sl_multiplier = 200
saved_sl_period = 1000
saved_ma_fast_period = 39
saved_ma_mid_period = 45
saved_ma_slow_period = 50
saved_short_target_multiple = 0.0
saved_long_target_multiple =  0.0
saved_short_entry_multiple =  0.0
saved_long_entry_multiple =   0.0
saved_will_frac_period = 18
...
suggestion_functions_ma = [
    # Features - MA
    suggest_ma_fast_period,
    suggest_ma_slow_period
]

suggestion_functions_entry_rsi = [
    # Features - RSI
...
suggestion_functions_exit_rsi = [
    # Features - RSI
    suggest_rsi_exit_lower_threshold,
    suggest_rsi_exit_upper_threshold,
    suggest_ma_mid_period
    #suggest_rsi_period
]

if str(testing_stage).endswith(('1','3','5','7','9')):
    chosen_functions_core = random.sample(suggestion_functions_core, 2)
    chosen_functions_ma = random.sample(suggestion_functions_ma, 2)
elif str(testing_stage).endswith(('0','2','4','6','8')):
    chosen_functions_entry_rsi = random.sample(suggestion_functions_entry_rsi, 3)
    chosen_functions_exit_rsi = random.sample(suggestion_functions_exit_rsi, 3)
...
ma_green_under_red = (ma_slow <= ma_fast)
ma_green_over_red = (ma_slow >= ma_fast)

# Returns
ma_slow, ma_mid, ma_fast, ma_long, ma_short, ma_green_under_red, ma_green_over_red
...
expr = """
RelativeStrengthIndex[rsi_exit]:

# RSI Indicator
rsi_exit_above_upper_threshold, rsi_exit_below_lower_threshold = get_rsi(@in_LTF_close, 
                                                                @p_rsi_period,
                                                                @p_rsi_exit_upper_threshold, 
                                                                @p_rsi_exit_lower_threshold)

# Returns
rsi_exit_above_upper_threshold, rsi_exit_below_lower_threshold

"""

RelativeStrengthIndex_exit = vbt.IF.from_expr(
    expr,
    takes_1d=True,
...
if __name__ == '__main__':       
    ...
    if str(testing_stage).endswith(('0','2','4','6','8')):
        n_trials = 2000 + (testing_stage * 50)
