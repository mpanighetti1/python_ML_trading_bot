...
for i in range(num_tests):
    command = ["python", "lps.py"]
    # For the second run onwards, pass the saved variables as input
    if i > 0:
        command.append("saved_variables.json")

    # Here we set the creationflags to create a new process with higher priority
    process = subprocess.Popen(command, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP | 32768)
    process.communicate() # Wait for process to complete

    # Optional sleep time to give some buffer between runs
    # time.sleep(2)
