...
def suggest_will_frac_period(trial):
    return ("will_frac_period", trial.suggest_int("will_frac_period", 2, 20, step=1))
def suggest_ma_fast_period(trial):
    return ("ma_fast_period", trial.suggest_int("ma_fast_period", 20, ma_mid_period - 20, step=20))
def suggest_ma_mid_period(trial):
    return ("ma_mid_period", trial.suggest_int("ma_mid_period", ma_fast_period + 20, ma_slow_period - 20, step=20))
def suggest_ma_slow_period(trial):
    return ("ma_slow_period", trial.suggest_int("ma_slow_period", ma_mid_period + 20, 2000, step=20))
def suggest_target_lookback_input(trial):
    return ("target_lookback_input", trial.suggest_int("target_lookback_input", 20, 1000, step=20))
def suggest_long_stop_period(trial):
    return ("long_stop_period", trial.suggest_int("long_stop_period", 20, 500, step=20))
def suggest_short_stop_period(trial):
    return ("short_stop_period", trial.suggest_int("short_stop_period", 20, 500, step=20))
...
def suggest_long_entry_multiple(trial):
    return ("long_entry_multiple", trial.suggest_float("long_entry_multiple", 0.0, 0.001, step=0.00001))
def suggest_rsi_period(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 20, 1500, step=20))
...
def objective(trial, LTF_high, 
    ...
    if str(testing_stage).endswith(('1','4','7','0')): 
        for i in range(5):
            name, variable = chosen_functions_all1[i](trial)
...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_testnew_4_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_testnew_4_{testing_stage}"  # Use the testing_stage in study name
...
def save_selected_variables(selected_params, testing_stage):
    # Load existing data
    try:
        with open('saved_variables_count_4.json', 'r') as f:
    ...
    # Update the 'testing_stage'
    data_to_save['testing_stage'] = testing_stage

    # Save the updated data
    with open('saved_variables_count_4.json', 'w') as f:
...
suggestion_functions_all1 = [
    suggest_target_lookback_input,
    suggest_long_stop_period,
    suggest_long_stop_multiplier,
    suggest_short_stop_period,
    suggest_short_stop_multiplier
]
...
if str(testing_stage).endswith(('1','4','7','0')):
    chosen_functions_all1 = random.sample(suggestion_functions_all1, 5)
elif str(testing_stage).endswith(('2','5','8')):
...
if str(testing_stage).endswith(('1','4','7','0')):
    chosen_functions_all1 = random.sample(suggestion_functions_all1, 3)
elif str(testing_stage).endswith(('2','5','8')):
    chosen_functions_all2 = random.sample(suggestion_functions_all2, 2)
elif str(testing_stage).endswith(('3','6','9')):
    chosen_functions_all3 = random.sample(suggestion_functions_all3, 3)
'''
###########################################################################
############################### Main Program ##############################
###########################################################################
...
if __name__ == '__main__':
    ...
    if best_params and top_trial_info:
        ...
        # [1] is trade_count, [0] is total_return.
        if saved_value is None or top_trial_info["objectives"][1] > saved_value:
            ...
            save_selected_variables(PARAM_TO_GLOBAL_VAR_MAP.keys(), testing_stage) 
            ...
            testing_stage = globals().get('testing_stage', 0) + 1
