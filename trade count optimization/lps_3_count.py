...
# Core
saved_long_stop_period = 40
saved_long_stop_multiplier = 10.0
saved_short_stop_period = 20
saved_short_stop_multiplier = 43.0
saved_target_lookback_input = 20
saved_rsi_entry_lower_threshold = 96
saved_rsi_period = 20
saved_rsi_entry_upper_threshold = 56
saved_ma_fast_period = 220
saved_ma_mid_period = 260
saved_ma_slow_period = 280
...
def suggest_ma_slow_period(trial):
    return ("ma_slow_period", trial.suggest_int("ma_slow_period", ma_mid_period + 5, 3000, step=1))
def suggest_target_lookback_input(trial):
    return ("target_lookback_input", trial.suggest_int("target_lookback_input", 2, 1000, step=1))
def suggest_long_stop_period(trial):
    return ("long_stop_period", trial.suggest_int("long_stop_period", 2, 1000, step=1))
def suggest_short_stop_period(trial):
    return ("short_stop_period", trial.suggest_int("short_stop_period", 2, 1000, step=1))
def suggest_long_stop_multiplier(trial):
    return ("long_stop_multiplier", trial.suggest_float("long_stop_multiplier", 1.5, 100.0, step=0.1))
def suggest_short_stop_multiplier(trial):
    return ("short_stop_multiplier", trial.suggest_float("short_stop_multiplier", 1.5, 100.0, step=0.1))
...
def suggest_rsi_period(trial):
    return ("rsi_period", trial.suggest_int("rsi_period", 2, 1500, step=1))
...
def objective(trial, LTF_high, 
    ...
    name, variable = chosen_functions_minors(trial)
...
def filter_and_sort_trials(study: opt.study.Study) -> list:
    ...
    filtered_trials = [
        ...
        if t.values[0] > 0 # if total_return is > 0.
        #t.values[1] >= 1 and 
        #t.values[2] <= 30000 # avg trade duration
    ]
    # Sort trials based on the first objective value in descending order, [1] = count, [0] = total_return
    if param_to_sort:
        sorted_trials = sorted(filtered_trials, key=lambda trial: (trial.values[1], trial.params[param_to_sort])
        if trial.values else (float('-inf'), float('inf')), reverse=True)
    else:
        sorted_trials = sorted(filtered_trials, key=lambda trial: trial.values[1] if trial.values else float('-inf'), reverse=True)
...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_testnew_3_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_testnew_3_{testing_stage}"  # Use the testing_stage in study name
    ...
    optimize_sampler = opt.samplers.BruteForceSampler()
...
def save_selected_variables(selected_params):
    # Load existing data
    try:
        with open('saved_variables_count_3.json', 'r') as f:
    ...
    with open('saved_variables_count_3.json', 'w') as f:
...
if (testing_stage == 15):    
    chosen_functions_minors = suggest_short_target_multiple
elif (testing_stage == 14):    
    chosen_functions_minors = suggest_long_target_multiple
elif (testing_stage == 13):
    chosen_functions_minors = suggest_short_entry_multiple
elif (testing_stage == 12):
    chosen_functions_minors = suggest_long_entry_multiple 
elif (testing_stage == 11):
    chosen_functions_minors = suggest_short_stop_multiplier
    param_to_sort = 'short_stop_multiplier'
elif (str(testing_stage).endswith(('1'))):
    chosen_functions_minors = suggest_ma_slow_period
elif (str(testing_stage).endswith(('2'))):
    chosen_functions_minors = suggest_ma_mid_period
elif (str(testing_stage).endswith(('3'))):
    chosen_functions_minors = suggest_ma_fast_period
elif (str(testing_stage).endswith(('4'))):
    chosen_functions_minors = suggest_target_lookback_input
elif (str(testing_stage).endswith(('5'))):
    chosen_functions_minors = suggest_rsi_period
elif (str(testing_stage).endswith(('6'))):
    chosen_functions_minors = suggest_rsi_entry_upper_threshold
elif (str(testing_stage).endswith(('7'))):
    chosen_functions_minors = suggest_rsi_entry_lower_threshold
elif (str(testing_stage).endswith(('8'))):
    chosen_functions_minors = suggest_long_stop_period
    param_to_sort = 'long_stop_period'
elif (str(testing_stage).endswith(('9'))):
    chosen_functions_minors = suggest_short_stop_period
    param_to_sort = 'short_stop_period'
elif (str(testing_stage).endswith(('0'))):
    chosen_functions_minors = suggest_long_stop_multiplier
    param_to_sort = 'long_stop_multiplier'
...
if __name__ == '__main__':
    ...
    if best_params and top_trial_info:
        if saved_value is None or top_trial_info["objectives"][1] > saved_value:
            
            saved_objective_value = top_trial_info["objectives"][1]
