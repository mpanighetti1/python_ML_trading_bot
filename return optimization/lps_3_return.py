...
def filter_and_sort_trials(study: opt.study.Study) -> list:
    ...
    if param_to_sort:
        sorted_trials = sorted(filtered_trials, key=lambda trial: (trial.values[0], trial.params[param_to_sort])
...
def run_optimization(testing_stage, LTF_high, 
    ...
    sqlite_path = f"./tests/op_return_3_{testing_stage}.db"
    storage = f"sqlite:///{sqlite_path}"

    study_name = f"op_return_3_{testing_stage}"  # Use the testing_stage in study name
...
def save_selected_variables(selected_params):
    # Load existing data
    try:
        with open('saved_variables_return_3.json', 'r') as f:
    ...
    with open('saved_variables_return_3.json', 'w') as f:
...
if (str(testing_stage).endswith(('1'))):
