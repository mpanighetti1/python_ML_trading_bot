...
num_tests = 7  # Number of times you want to run the tests
...
for i in range(num_tests):
    ...
    command = ["python", "lps_3_return.py"]
    # For the second run onwards, pass the saved variables as input
    if i > 0:
        command.append("saved_variables_return_3.json")

    # Here we set the creationflags to create a new process with higher priority
    process = subprocess.Popen(command, creationflags=subprocess.CREATE_NEW_PROCESS_GROUP | 128)
